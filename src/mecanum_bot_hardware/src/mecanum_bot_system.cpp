#include "mecanum_bot_hardware/mecanum_bot_hardware.hpp"
...
public:
  ...
  : Node("mecanum_bot_system_node", options)
  {
    RCLCPP_INFO(get_logger(), "Starting MecanumBotSystemNode...");
    
    // Create hardware interface
    hardware_interface_ = std::make_shared<mecanum_bot_hardware::MecanumBotSystemHardware>();
    
    // Initialize hardware interface
    hardware_interface::HardwareInfo hardware_info;
    
    // Configure hardware info from parameters
    hardware_info.name = "mecanum_bot_system";
    ...
    this->declare_parameter<int>("node_id_base", 1);
    ...
    // std::unique_ptr<hardware_interface::SystemInterface> hardware_ptr(hardware_interface_.get());
    // resource_manager_->import_component(std::move(hardware_ptr), hardware_info);
...
private:
  std::shared_ptr<mecanum_bot_hardware::MecanumBotSystemHardware> hardware_interface_;
...
int main(int argc, char** argv)
{
  ...
  executor.add_node(node->controller_manager_);
