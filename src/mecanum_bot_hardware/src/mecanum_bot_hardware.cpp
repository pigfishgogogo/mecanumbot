...
namespace mecanum_bot_hardware
{
...
CallbackReturn MecanumBotSystemHardware::on_init(const hardware_interface::HardwareInfo & info)
{
  ...
  for (size_t i = 0; i < info_.joints.size(); i++)
  {
    ...
    std::cout << "Joint " << i << ": " << joint_names_[i] 
              << ", Wheel radius: " << wheel_radius_[i] << std::endl;
  }
  ...
  std::vector<double> & wheel_velocities)
{
  ...
  const double l = 0.2;  // distance from center to wheel along x (adjust as needed)
  const double w = 0.2;  // distance from center to wheel along y (adjust as needed)
  ...
  wheel_velocities[3] = linear_x - linear_y + (l + w) * angular_z;  // rear_right
}
  ...
  double & linear_x, double & linear_y, double & angular_z)
{
  ...
  const double l = 0.2;  // distance from center to wheel along x
  const double w = 0.2;  // distance from center to wheel along y
...
PLUGINLIB_EXPORT_CLASS(
  mecanum_bot_hardware::MecanumBotSystemHardware,
